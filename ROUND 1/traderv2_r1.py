...
class Trader:
    ...
    def trade_pepper(self, state: TradingState, pepper_prices: list) -> list:
        # INTARIAN_PEPPER_ROOT
        # ── HYPERPARAMS ──────────────────────────────
        DRIFT_WINDOW    = 200   # timestamps pour estimer le drift
        DRIFT_THRESHOLD = 0.05   # ticks/timestamp minimum pour uptrend
        ASK_SKEW        = 3      # ticks supplémentaires sur le ask en uptrend
        # ─────────────────────────────────────────────
        ...
        if len(order_depth.buy_orders) == 0 or len(order_depth.sell_orders) == 0:
            return pepper_prices

        fair_value = self.get_bbo_mid(state, product)
        if fair_value is None:
            return pepper_prices

        # ── Mise à jour historique ──
        pepper_prices.append(fair_value)
        if len(pepper_prices) > DRIFT_WINDOW + 1:
            pepper_prices = pepper_prices[-(DRIFT_WINDOW + 1):]

        # ── Estimation drift → skew ask ──
        ask_skew = 0
        if len(pepper_prices) >= DRIFT_WINDOW:
            drift = (pepper_prices[-1] - pepper_prices[-DRIFT_WINDOW]) / DRIFT_WINDOW
        ...
        # Skew asymétrique : monte le ask pour accumuler du long
        sell_price += ask_skew
        ...
        if not (pos > 0 and buy_price >= math.floor(fair_value)):
            self.send_buy_order(product, buy_price, max_buy,
                msg=f"PEPPER MM Buy {max_buy} @ {buy_price} skew={ask_skew}")

        if not (pos < 0 and sell_price <= math.ceil(fair_value) + ask_skew):
            self.send_sell_order(product, sell_price, max_sell,
                msg=f"PEPPER MM Sell {max_sell} @ {sell_price} skew={ask_skew}")
    ...
    def run(self, state: TradingState):
        ...
        if state.traderData and state.traderData != "":
            ...
            pepper_prices: list = saved.get("pepper_prices", [])
        else:
            pepper_prices: list = []
        ...
        if "INTARIAN_PEPPER_ROOT" in state.order_depths:
            pepper_prices = self.trade_pepper(state, pepper_prices)

        # ── Sauvegarder l'état ──
        trader_data = json.dumps({"pepper_prices": pepper_prices})
        conversions = 0
